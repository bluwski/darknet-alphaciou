...
void set_track_id(detection *new_dets, int new_dets_num, float thresh, float sim_thresh, float track_ziou_norm, int deque_size, int dets_for_track, int dets_for_show)
{
    ...
    for (int index = 0; index < new_dets_num*old_dets.size(); ++index) {
        ...
        //printf(" ziou = %f \n", box_ziou(new_dets[new_id].bbox, old_dets[old_id].bbox));
        if (track_idx[track_id] && new_idx[new_id] && old_idx[old_id] && check_classes_id(new_dets[new_id], old_dets[old_id], thresh)) {
            ...
            //float ziou = box_ziou(new_dets[new_id].bbox, old_dets[old_id].bbox);
            float ziou = box_iou(new_dets[new_id].bbox, old_dets[old_id].bbox);
            sim = sim * (1 - track_ziou_norm) + ziou * track_ziou_norm;
