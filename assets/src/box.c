...
{
    //IOU, QIOU, MSE, DIOU, ZIOU
    switch(iou_kind) {
        case IOU: return box_iou(a, b);
        case QIOU: return box_qiou(a, b);
        case DIOU: return box_diou(a, b);
        case ZIOU: return box_ziou(a, b);
    }
    return box_iou(a, b);
}
...
float box_iou(box a, box b)
{
    ...
    return (pow(I / U, alpha_iou));
}

float box_qiou(box a, box b)
{
    boxabs ba = box_c(a, b);
    int alpha_qiou = 2;
    ...
    float u = box_union(a, b);
    float qiou_term = pow((c - u) / c, alpha_qiou);
#ifdef DEBUG_PRINTS
    printf("  c: %f, u: %f, qiou_term: %f\n", c, u, qiou_term);
#endif
    return iou - qiou_term;
}
...
float box_ziou(box a, box b)
{
    ...
    float d = pow(u,alpha_ziou) / pow(c,alpha_ziou);
    float ar_gt = b.w / b.h;
    ...
    float ziou_term = pow(d, alpha_ziou) + pow(alpha * ar_loss, alpha_ziou);                   //ziou
#ifdef DEBUG_PRINTS
    printf("  c: %f, u: %f, riou_term: %f\n", c, u, ziou_term);
#endif
    return iou - ziou_term;
}
...
dxrep dx_box_iou(box pred, box truth, IOU_LOSS iou_loss) {
 boxabs pred_tblr = to_tblr(pred);
...
#ifdef DEBUG_PRINTS
    printf("\niou: %f, qiou: %f\n", box_iou(pred, truth), box_qiou(pred, truth));
    ...
    float qiou_Cw = fmax(pred_r, truth_tblr.right) - fmin(pred_l, truth_tblr.left);
    float qiou_Ch = fmax(pred_b, truth_tblr.bot) - fmin(pred_t, truth_tblr.top);
    float qiou_C = qiou_Cw * qiou_Ch;
    //float IoU = I / U;
//#ifdef DEBUG_PRINTS
    //printf("X: %f", X);
    //printf(", Xhat: %f", Xhat);
...
//#endif
    ...
    float dC_wrt_t = pred_t < truth_tblr.top ? (-1 * qiou_Cw) : 0;
    float dC_wrt_b = pred_b > truth_tblr.bot ? qiou_Cw : 0;
    float dC_wrt_l = pred_l < truth_tblr.left ? (-1 * qiou_Ch) : 0;
    float dC_wrt_r = pred_r > truth_tblr.right ? qiou_Ch : 0;
    ...
    int alpha_qiou = 2;
      if (qiou_C > 0) {
        ...
        p_dt += pow((qiou_C * dU_wrt_t) - (U * dC_wrt_t),alpha_qiou) / pow(qiou_C * qiou_C,alpha_qiou);
        p_db += pow((qiou_C * dU_wrt_b) - (U * dC_wrt_b),alpha_qiou) / pow(qiou_C * qiou_C,alpha_qiou);
        p_dl += pow((qiou_C * dU_wrt_l) - (U * dC_wrt_l),alpha_qiou) / pow(qiou_C * qiou_C,alpha_qiou);
        p_dr += pow((qiou_C * dU_wrt_r) - (U * dC_wrt_r),alpha_qiou) / pow(qiou_C * qiou_C,alpha_qiou);
      }
	  if (Iw<=0||Ih<=0) {
		p_dt = pow((qiou_C * dU_wrt_t) - (U * dC_wrt_t),alpha_qiou) / pow(qiou_C * qiou_C,alpha_qiou);
        p_db = pow((qiou_C * dU_wrt_b) - (U * dC_wrt_b),alpha_qiou) / pow(qiou_C * qiou_C,alpha_qiou);
	...
	//The following codes are calculating the gradient of ziou.
	...
	float ar_dw=8/(M_PI*M_PI)*(atan(ar_gt)-atan(ar_pred))*pred.h;
        ...
        if (C > 0) {
        // dar*
            p_dx += pow(2*(truth.x-pred.x)*C-(2*Cw*dCw_dx+2*Ch*dCh_dx)*S,alpha_ziou) / (pow(C * C, alpha_ziou));
            p_dy += pow(2*(truth.y-pred.y)*C-(2*Cw*dCw_dy+2*Ch*dCh_dy)*S,alpha_ziou) / (pow(C * C, alpha_ziou));
            p_dw += pow((2*Cw*dCw_dw+2*Ch*dCh_dw)*S,alpha_ziou) / pow(C * C,alpha_ziou) + (pow(alpha * ar_dw,alpha_ziou));
            p_dh += pow((2*Cw*dCw_dh+2*Ch*dCh_dh)*S,alpha_ziou) / pow(C * C,alpha_ziou) + (pow(alpha * ar_dh,alpha_ziou));
        }
	if (Iw<=0||Ih<=0){
            p_dx = pow(2*(truth.x-pred.x)*C-(2*Cw*dCw_dx+2*Ch*dCh_dx)*S,alpha_ziou) / (pow(C * C,alpha_ziou));
            p_dy = pow(2*(truth.y-pred.y)*C-(2*Cw*dCw_dy+2*Ch*dCh_dy)*S,alpha_ziou) / (pow(C * C,alpha_ziou));
            p_dw = pow((2*Cw*dCw_dw+2*Ch*dCh_dw)*S,alpha_ziou) / pow((C * C) + alpha * ar_dw, alpha_ziou);
            p_dh = pow((2*Cw*dCw_dh+2*Ch*dCh_dh)*S,alpha_ziou) / pow((C * C) + alpha * ar_dh,alpha_ziou);
        }
    }
