...
ious delta_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, float *delta, float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss, int accumulate, float max_delta, int *rewritten_bbox, int new_coords)
{
    ...
    all_ious.qiou = box_qiou(pred, truth);
    all_ious.diou = box_diou(pred, truth);
    all_ious.ziou = box_ziou(pred, truth);
...
{
    {
        ...
        //float tot_qiou_loss = 0;
        float tot_diou_loss = 0;
        float tot_ziou_loss = 0;
        float recall = 0;
        float recall75 = 0;
        float avg_cat = 0;
        float avg_obj = 0;
        ...
        for (t = 0; t < l.max_boxes; ++t) {
            ...
            if (mask_n >= 0) {
                ...
                // range is -1 <= qiou <= 1
                tot_qiou += all_ious.qiou;
                args->tot_qiou_loss += 1 - all_ious.qiou;

                tot_diou += all_ious.diou;
                tot_diou_loss += 1 - all_ious.diou;

                tot_ziou += all_ious.ziou;
                tot_ziou_loss += 1 - all_ious.ziou;
            ...
            for (n = 0; n < l.total; ++n) {
                ...
                if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f) {
                    ...
                    pred.w = l.biases[2 * n] / state.net.w;
                    pred.h = l.biases[2 * n + 1] / state.net.h;
                    float iou = box_iou_kind(pred, truth_shift, l.iou_thresh_kind); // IOU, QIOU, MSE, DIOU, ZIOU
                    // iou, n
...
void forward_yolo_layer(const layer l, network_state state)
{
    ...
    *(l.cost) = 0;
    ...
    for (b = 0; b < l.batch; b++)
    {
        ...
        tot_qiou_loss += yolo_args[b].tot_qiou_loss;
    ...
    if (l.show_details == 0) {
        ...
        *(l.cost) = loss;
        ...
        fprintf(stderr, "v3 (%s loss, Normalizer: (iou: %.2f, obj: %.2f, cls: %.2f) Region %d Avg (IOU: %f), count: %d, total_loss = %f \n",
            (l.iou_loss == MSE ? "mse" : (l.iou_loss == QIOU ? "qiou" : "iou")), l.iou_normalizer, l.obj_normalizer, l.cls_normalizer, state.index, tot_iou / count, count, loss);
    }
    else {
        ...
        *(l.cost) = loss;
        ...
        if (l.iou_loss == MSE) {
            *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
            ...
            if (l.iou_loss == QIOU) {
                avg_iou_loss = count > 0 ? l.iou_normalizer * (tot_qiou_loss / count) : 0;
            }
            else {
        ...
        fprintf(stderr, "v3 (%s loss, Normalizer: (iou: %.2f, obj: %.2f, cls: %.2f) Region %d Avg (IOU: %f), count: %d, class_loss = %f, iou_loss = %f, total_loss = %f \n",
            (l.iou_loss == MSE ? "mse" : (l.iou_loss == QIOU ? "qiou" : "iou")), l.iou_normalizer, l.obj_normalizer, l.cls_normalizer, state.index, tot_iou / count, count, classification_loss, iou_loss, loss);

        //fprintf(stderr, "v3 (%s loss, Normalizer: (iou: %.2f, cls: %.2f) Region %d Avg (IOU: %f, QIOU: %f), Class: %f, Obj: %f, No Obj: %f, .5R: %f, .75R: %f, count: %d, class_loss = %f, iou_loss = %f, total_loss = %f \n",
        //    (l.iou_loss == MSE ? "mse" : (l.iou_loss == QIOU ? "qiou" : "iou")), l.iou_normalizer, l.obj_normalizer, state.index, tot_iou / count, tot_qiou / count, avg_cat / class_count, avg_obj / count, avg_anyobj / (l.w*l.h*l.n*l.batch), recall / count, recall75 / count, count,
